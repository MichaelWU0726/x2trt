...
from torch2onnx.utils import non_max_suppression, make_divisible, scale_coords, xyxy2xywh
from torch2onnx.utils import color_list, plot_one_box
from torch2onnx.utils import letterbox
...
class autoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        for i, im in enumerate(imgs):
            ...
            if isinstance(im, str):  # filename
                # if im.startswith('http'):
                #     im = requests.get(im, stream=True).raw
                im, f = np.asarray(Image.open(im)), im
